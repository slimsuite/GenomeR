...
# setting defaults incase of null
#if(is.null(params$simple_min_kmer)) {
#    simple_min = 7
#} else {
#    simple_min = params$simple_min_kmer
#}
#
#if(is.null(params$simple_max_kmer)){
#    simple_max = 100
#} else {
#    simple_max = params$simple_max_kmer
#}

if(is.null(params$peak_min_kmer)) {
    peak_min = 7
} else {
    peak_min = params$peak_min_kmer 
}

if(is.null(params$peak_max_kmer)){
    peak_max = 100
} else {
    peak_max = params$peak_max_kmer
}

if(is.null(params$gscope_max_kmer)) {
   gscope_max = 100
} else {
    gscope_max = params$gscope_max_kmer
}
```


```{r genomeScopeFunctions, echo=FALSE, message=FALSE}
...
```{r gscopeCalc, echo=FALSE, message=FALSE}
 gscope_count = runGenomeScope(params$df, params$kmer_length, params$read_length, gscope_max,
                               params$gscope_rounds,
...
```{r simpleCalc, echo=FALSE}
if(is.null(params$simple_min_kmer)|| is.null(params$simple_max_kmer)) {
    simple_count = simple_count_kmer(params$df, show_error = show)
} else {
    simple_count = simple_count_kmer(params$df, params$simple_min_kmer, params$simple_max_kmer, show_error = show)
}
...
if(is.null(params$peak_max_kmer) || is.null(params$peak_min_kmer)) {
    peak_count = peak_count_kmer(params$df, show_error = show, num_peaks)
} else {
    peak_count = peak_count_kmer(params$df, params$peak_min_kmer, params$peak_max_kmer, show_error = show, num_peaks)
}
...
kable(summaryTable(), caption="Size Estimations", align='l')


indf<- data.frame(
    Input = c("Input Type", "K-mer Length", "Read Length"),
    Value = c(params$input_type, params$kmer_length, params$read_length)
)

colnames(indf) <- c("Input", "Value")

kable(indf, caption="Input Summary", align = 'l')
...
indf <- data.frame(
    Input = c("K-mer Cutoff", "Number of Rounds", "Start Shift", "Error Cutoff", "Maximum Iteration", 
              "Score Difference %", "Heterozygosity Fold Difference"),
    Value = c(gscope_max, params$gscope_rounds, params$gscope_start_shift, params$gscope_error_cuttoff,
              params$gscope_max_iter, params$gscope_score_close, params$gscope_het_diff)
)
colnames(indf)<- c("Input", "Value")

kable(indf, caption="Summary of Input for GenomeScope", align = 'l')
```
...
### Output
```{r, echo=FALSE}
kable(gscope_count$summary, caption = "GenomeScope Output", align = 'l')
```
...
### Input
```{r simpleInput, echo=FALSE}
library(knitr)

if(is.null(params$simple_max_kmer) || is.null(params$simple_min_kmer)) {
    simple_min = calc_start_freq(params$df)
    simple_max = 100
} else {
    simple_min = params$simple_min_kmer
    simple_max = params$simple_max_kmer
...
indf <- data.frame(
    Input = c("K-mer Cutoff", "Error Cutoff"),
    Value = c(simple_max, simple_min)
)
colnames(indf)<- c("Input", "Value")

kable(indf, caption="Summary of Input for Simple Count", align = 'l')
```
...
### Input
```{r peakInput, echo=FALSE}
library(knitr)

if(is.null(params$peak_max_kmer) || is.null(params$peak_min_kmer)) {
    peak_min = calc_start_freq(params$df)
    peak_max = 100
} else {
    peak_min = params$peak_min_kmer
    peak_max = params$peak_max_kmer
...
indf <- data.frame(
    Input = c("K-mer Cutoff", "Error Cutoff", "Ploidy"),
    Value = c(peak_max, peak_min, params$diploid)
)
colnames(indf)<- c("Input", "Value")

kable(indf, caption="Summary of Input for Peak Frequency", align = 'l')
```
...
## Multiple Cutoffs
```{r cuttoff, echo=FALSE, message=FALSE}

cutoff_sizes <- function(df) {
    ...
    for (x in perc) {
        ...
        g = runGenomeScope(df, params$kmer_length, params$read_length, max_kmer, 
                           params$gscope_rounds, params$gscope_start_shift, params$gscope_error_cuttoff, 
                           params$gscope_max_iter, params$gscope_score_close, params$gscope_het_diff)
        s = simple_count_kmer(df, params$min_kmer, max_kmer, show_error=TRUE)
        p = peak_count_kmer(df, params$min_kmer, max_kmer, show_error=TRUE, num_peaks=1)
    ...
    data = data.frame(cutoff, perc, gscope, peak, simple)
                
    p = plot_ly(data, x= ~cutoff, y= ~gscope, name = "Genome Scope", type="scatter", mode="lines")
    p = add_trace(p, x= ~cutoff, y= ~peak, name = "Peak Kmer", type="scatter", mode="lines")
    p = add_trace(p, x= ~cutoff, y= ~simple, name = "Simple Count", type="scatter", mode="lines")
    p = layout(p, showlegend = TRUE, xaxis=list(title='Max kmer cutoff'), 
               yaxis=list(title='Genome Size'))
    p$elementId = NULL  #TODO temp approach to suppress warning
...
#cutSize = cutoff_sizes(params$df)

#cutSize$p

#kable(cutSize$data, caption="Size Estimation with Multiple K-mer Cutoffs")
```
